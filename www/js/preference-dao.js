var PreferenceDAO = function () {

    this.initialize = function () {
        this.db = DBFactory.getInstance().db;
        //this.db = dbObject.database;
        var deferred = $.Deferred();
        // this.db = window.openDatabase("PreferenceDB", "1.0", "Preference local Database", 200000);
        // this.db.transaction(
        //     function (tx) {
        //         createDbSchema(tx);
        //     },
    ...
    this.insert = function (user) {
        ...
        this.db.transaction(
            function (tx) {
                var sql = "INSERT INTO USER (user_id, last_name, first_name, birth_date, resting_heart_rate, max_heart_rate, email) " +
                    "VALUES (?,?,?,?,?,?,?)";

                tx.executeSql(sql, [user.userId, user.lastName, user.firstName, user.birthDate, user.restingHeartRate, user.maxHeartRate, user.email],
                    function (tx) {
                        deferred.resolve();
                });
            },
